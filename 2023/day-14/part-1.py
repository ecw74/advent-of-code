...
def rotate_matrix_90_counterclockwise(matrix):
    """
    Rotates a 2D matrix (list of lists) 90 degrees counterclockwise.

    Args:
    matrix (List[List[str]]): A 2D matrix to be rotated.

    Returns:
    List[List[str]]: The rotated matrix.
    """
    # Transpose the matrix and reverse the entire transposed matrix
...
def process_grid(grid):
    """
    Processes the grid by moving all rounded rocks north.
    ...
    processed_columns = (move_rock_right(column) for column in processed_columns)
    processed_columns = rotate_matrix_90_counterclockwise(processed_columns)
...
if __name__ == "__main__":
    ...
    total_load = calculate_load(processed_grid)
