...
def count_removable(supports, supportedBy):
    """
    Count the number of bricks that can be removed from a structure without affecting its stability.

    Args:
    supports (Dict[int, List[int]]): A dictionary mapping each brick ID to a list of IDs of bricks it supports.
    supportedBy (Dict[int, List[int]]): A dictionary mapping each brick ID to a list of IDs of bricks supporting it.

    Returns:
    The number of bricks that can be removed.
    """
    return sum(1 for bId in supports if all(len(supportedBy[sId]) > 1 for sId in supports[bId]))
...
if __name__ == '__main__':
    ...
    count = count_removable(supports, supportedBy)
